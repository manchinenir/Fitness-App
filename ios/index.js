...
const LOGO_CID = "flex-facility-logo";
...
let _logoAttachment = undefined;

function getLogoAttachment() {
  if (_logoAttachment !== undefined) {
    return _logoAttachment;
  }

  try {
    const candidateLogoPaths = [
      path.join(__dirname, "assets", "splash_logo.png"),
      path.join(__dirname, "..", "assets", "splash_logo.png"),
      path.join(__dirname, "..", "assets", "images", "flex_login", "logo.png"),
    ];
    const logoPath = candidateLogoPaths.find((candidate) => fs.existsSync(candidate));

    if (!logoPath) {
      throw new Error("No local logo asset found for email template");
    }
    ...
    console.error("Failed to load inline logo for emails:", error.message);
...
function getLogoImgSrc() {
  return getLogoAttachment() ? `cid:${LOGO_CID}` : LOGO_URL;
}
...
function sendTransactionalEmail({ to, subject, text, html, fromName }) {
  ...
  const inlineLogo = html ? getLogoAttachment() : null;
          ...
          <table width="100%" cellpadding="0" cellspacing="0" style="background:${BRAND_COLORS.cardBg};border-radius:16px;box-shadow:0 4px 16px rgba(15,23,42,0.08);overflow:hidden;">
            <tr>
              <td style="padding:20px 24px 12px 24px;border-bottom:1px solid #e5e7eb;">
                <img src="${logoImgSrc}" alt="Flex Facility" width="120" style="display:block;margin-bottom:16px;" />
    ...
    const logoImgSrc = getLogoImgSrc();
        ...
        const emailHtml = `
          ...
          <div style="text-align:center;margin-bottom:12px;">
            <img src="${logoImgSrc}" alt="Flex Facility" width="120" style="display:inline-block;" />
          </div>
 ...
 * Finds slots starting in the next 23–25 hours and sends a push notification
...
exports.sendSessionReminders = functions
  ...
  .onRun(async () => {
    ...
    // Window: slots starting between 23h and 25h from now (1-day reminder)
    ...
    for (const slotDoc of slotsSnap.docs) {
      ...
      // Collect FCM tokens for all booked clients
      const tokens = [];
      for (const uid of bookedBy) {
        try {
          ...
          console.error(`Failed to fetch token for uid ${uid}:`, e);
        }
      }
      ...
      const message = {
        notification: {
          ...
          body: `You have a session tomorrow at ${slotTime} with ${trainer}.`,
        },
        data: {
          slotDate,
          slotTime,
      ...
      try {
        ...
        console.log(
          `Reminders sent for slot ${slotDoc.id}: ${response.successCount} success, ${response.failureCount} failed`
        );
      } catch (e) {
        console.error(`Error sending reminders for slot ${slotDoc.id}:`, e);
      }
    }
