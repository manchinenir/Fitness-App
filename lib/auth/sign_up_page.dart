...
class _SignupPageState extends State<SignupPage> {
  ...
  String selectedRole = 'client'; // 'client' or 'admin'
  String? referrerId;
  ...
  final List<String> allowedAdminEmails = const [
  ...
  /// ✅ Admin is valid if email is one of the allowed admin emails
  bool _isValidAdmin() {
  ...
  double _calculateBMI() {
    final feet =
        double.tryParse(heightFeetController.text.replaceAll(',', '').trim()) ??
            0.0;
    final inches = double.tryParse(
            heightInchesController.text.replaceAll(',', '').trim()) ??
        0.0;
    final weightLbs =
  ...
  Future<void> _processReferral(String newUserId, String newUserName) async {
    ...
    try {
      final querySnapshot = await _firestore
          ...
          .where(
            'referralCode',
            isEqualTo: referralCodeController.text.trim().toUpperCase(),
          )
          .limit(1)
          .get();
  ...
  Future<void> _signup() async {
    ...
    // ✅ If trying to sign up as ADMIN, validate by email
    if (selectedRole == 'admin' && !_isValidAdmin()) {
      setState(() {
        errorMessage = 'Only authorized admin emails can sign up.';
      });
      return;
    }
      ...
      if (user != null) {
        ...
        if (selectedRole == 'admin') {
          ...
          userData = {
            ...
            'referralCodeUsed': referralCodeValid
                ? referralCodeController.text.trim().toUpperCase()
                : null,
          };
        }
          ...
          SnackBar(
            content: Text(
              verificationEmailSent
                  ? 'Account created! Verification email sent. Check inbox or spam.'
                  : 'Account created, but verification email failed: ${verificationError.isEmpty ? 'try resend from login' : verificationError}',
            ),
            backgroundColor:
                verificationEmailSent ? Colors.green : Colors.orange,
        ...
        child: Form(
          ...
          child: Column(
            children: [
              ...
              Container(
                ...
                child: DropdownButtonFormField<String>(
                  ...
                  decoration: InputDecoration(
                    ...
                    prefixIcon:
                        Icon(Icons.account_circle, color: navyBlue),
              ...
              // From here down: ONLY for CLIENT
              if (selectedRole == 'client') ...[
                TextFormField(
                ...
                Container(
                  ...
                  child: DropdownButtonFormField<String>(
                    ...
                    decoration: InputDecoration(
                      ...
                      labelStyle: TextStyle(color: Colors.grey[700]),
                      prefixIcon:
                          Icon(Icons.person_outline, color: navyBlue),
                      border: InputBorder.none,
                    ),
                    items: const [
