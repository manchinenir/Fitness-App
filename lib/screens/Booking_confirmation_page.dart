...
class _BookingConfirmationPageState extends State<BookingConfirmationPage> {
  ...
  Future<Map<String, String>> _resolveSlotDocInfo(
    DateTime selectedDate,
    String selectedTime,
  ) async {
    final dateKey = DateFormat('yyyy-MM-dd').format(selectedDate);
    final primaryDocId = '$dateKey|$selectedTime';
    final fallbackTime = _normalizeSlotLabelTo24Hour(selectedTime);
    final fallbackDocId = '$dateKey|$fallbackTime';
    final col = FirebaseFirestore.instance.collection('trainer_slots');

    final primary = await col.doc(primaryDocId).get();
    if (primary.exists) {
      return {'docId': primaryDocId, 'time': selectedTime};
    }

    if (fallbackDocId != primaryDocId) {
      final fallback = await col.doc(fallbackDocId).get();
      if (fallback.exists) {
        return {'docId': fallbackDocId, 'time': fallbackTime};
      }
    }
  ...
  Future<void> _confirmBooking() async {
    ...
    final availableSessions = (_activePlan!['availableSessions'] as num?)?.toInt() ?? 0;
    ...
    if (availableSessions <= 0) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
            content:
                Text('No available sessions in your active plan.')),
      );
      return;
    }
    ...
    try {
      ...
      final actualTotal = (purchaseData?['totalSessions'] as num?)?.toInt() ?? 0;
      final actualBooked = (purchaseData?['bookedSessions'] as num?)?.toInt() ?? 0;
      final actualAvailable = actualTotal - actualBooked;
      ...
      if (actualAvailable <= 0) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
              content: Text(
                  'No available sessions. Please refresh and try again.')),
        );
        return;
      }
      ...
      print('   Available Sessions: $actualAvailable ($actualTotal total - $actualBooked booked)');
  ...
  Future<void> _processRescheduleWithoutPlan(User user) async {
    try {
      ...
      final userEmail = userDoc.data()?['email'] ?? '';
        ...
        final resolvedSlot =
          await _resolveSlotDocInfo(widget.selectedDate, widget.selectedTime);
        final newDocId = resolvedSlot['docId']!;
        final bookingTime = resolvedSlot['time']!;
      final newRef =
          FirebaseFirestore.instance.collection('trainer_slots').doc(newDocId);

        final startTimeRaw = bookingTime.split(' - ').first.trim();
        final parsedStart = _parseTimeFlexible(startTimeRaw);
      ...
      await FirebaseFirestore.instance.runTransaction((txn) async {
        ...
        Map<String, dynamic> statusByUserNew = {};
        ...
        if (newSnap.exists) {
          ...
          statusByUserNew =
              Map<String, dynamic>.from(d['status_by_user'] ?? {});
        }
        ...
        Map<String, dynamic>? statusByUserOld;
        ...
        if (oldSnap != null && oldSnap.exists) {
          ...
          // Preserve existing statuses for all other users
          statusByUserOld =
              Map<String, dynamic>.from(od['status_by_user'] ?? {});
          ...
          if (idx != -1) {
            ...
            // Update only this user's status on old slot
            statusByUserOld[user.uid] = 'Cancelled';
        ...
        // Update only this user's status on new slot
        statusByUserNew[user.uid] = 'Rescheduled';
        ...
        txn.set(
          newRef,
          {
            ...
            'status_by_user': statusByUserNew,
            ...
            'is_reschedule': true, // tells Cloud Function to send rescheduled email
          },
        ...
        if (oldRef != null && oldSnap != null && oldSnap.exists) {
          txn.update(oldRef, {
            ...
            'last_updated': FieldValue.serverTimestamp(),
            'status_by_user': statusByUserOld,
            'is_reschedule_cancel': true, // tells Cloud Function to skip cancel email
          });
        }
      });

      // Trainer email is sent by Cloud Functions; skip client-side duplicate sends.
  ...
  Future<void> _processBooking(
    ...
    try {
      ...
      await FirebaseFirestore.instance.runTransaction((txn) async {
        ...
        statusByUserNew[user.uid] = isRescheduleLocal ? 'Rescheduled' : 'Confirmed';
        ...
        txn.set(
          newRef,
          {
            ...
            'planName': planName,
            'planId': planId,
            if (isRescheduleLocal) 'is_reschedule': true, // tells Cloud Function to send rescheduled email
          },
          SetOptions(merge: true),
        );
        ...
        if (oldRef != null && oldSnap != null && oldSnap.exists) {
          txn.update(oldRef, {
            ...
            'last_updated': FieldValue.serverTimestamp(),
            'status_by_user': statusByUserOld,
            if (isRescheduleLocal) 'is_reschedule_cancel': true, // tells Cloud Function to skip cancel email
          });
        }
      });
