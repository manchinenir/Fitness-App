...
class _MySchedulePageState extends State<MySchedulePage> with SingleTickerProviderStateMixin {
  ...
  DateTime? _parseSlotDateTime(dynamic rawDate, dynamic rawTime) {
    DateTime slotDate;

    if (rawDate is Timestamp) {
      slotDate = rawDate.toDate().toLocal();
    } else if (rawDate is DateTime) {
      slotDate = rawDate.toLocal();
    } else if (rawDate is String) {
      try {
        slotDate = DateFormat('yyyy-MM-dd').parse(rawDate).toLocal();
      } catch (_) {
        return null;
      }
    } else {
    ...
    try {
      final startTimeStr = rawTime.contains(' - ')
          ? rawTime.split(' - ').first.trim()
          : rawTime.trim();
      final parsedTime = _parseTimeFlexible(startTimeStr);
  ...
  void initState() {
    ...
    // Re-sort every minute so slots move from Upcoming → Past as time passes
    _resortTimer = Timer.periodic(const Duration(minutes: 1), (_) {
      if (mounted) _resortSlots();
    });
  }
  ...
  void _resortSlots() {
    ...
    final past = _allSlots.where((s) => (s['date'] as DateTime).isBefore(now)).toList();
    final upcoming = _allSlots.where((s) => !(s['date'] as DateTime).isBefore(now)).toList();
    ...
    _firestore
      ...
      try {
        ...
        for (var doc in snapshot.docs) {
          ...
          final slotDateTime = _parseSlotDateTime(data['date'], timeString);

          if (timeString == null || slotDateTime == null) {
            debugPrint('Skipping slot with invalid date/time in doc ${doc.id}');
            continue;
          }
  ...
  Future<void> _cancelBooking(DocumentReference docRef) async {
    ...
    try {
      ...
      await _firestore.runTransaction((transaction) async {
        ...
        statusByUser[user.uid] = 'Cancelled';
