    ...
    try {
      await FirebaseFirestore.instance.runTransaction((transaction) async {
        ...
        if (!bookedBy.contains(currentUser.uid)) {
          // This case should ideally not happen if the UI accurately reflects bookings
          throw Exception('No booking found to cancel for this user in this slot');
        }
        ...
        bookedBy.removeWhere((uid) => uid == currentUser.uid);
      ...
      // Trigger a UI refresh to show the slot as available again
      setState(() {});
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        actions: [
          ...
          if (!isFull || isBookedByUser) // Show "Proceed" or "Cancel Booking" button if not full or if already booked by user
            TextButton(
              onPressed: () async {
                Navigator.pop(context); // Close the dialog

                if (isBookedByUser) {
                  // User wants to cancel their existing booking for this slot
                  await _cancelBooking(docId);
                } else {
                  // User wants to book a new slot
                  // If this is a reschedule operation, cancel the old slot FIRST
                  if (widget.rescheduleSlot != null) {
                    final oldSlotDocId = widget.rescheduleSlot!['id']; // Use 'id' to get the document ID
                    await _cancelBooking(oldSlotDocId);
                  }
                  ...
                  Navigator.push(
                    ...
                    // Refresh the UI after returning from the booking confirmation
                    // This is important whether booking was successful or not
                    if (result == true) { // Assuming BookingConfirmationPage returns true on successful booking
                      // Optionally, if it was a reschedule, pop back to MySchedulePage directly
                      // if the user successfully completed the reschedule.
                      // Navigator.pop(context); // uncomment if you want to go back to MySchedulePage immediately
                    }
                    setState(() {}); // Refresh current ClientBookSlot page state
                  });
                }
              },
