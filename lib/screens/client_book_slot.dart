...
class _ClientBookSlotState extends State<ClientBookSlot> {
  ...
  StreamSubscription<DocumentSnapshot<Map<String, dynamic>>>? _scheduleListener;
  Map<String, List<String>> _loadedWeeklySlots = {};
  Map<String, List<String>> _loadedDateOverrides = {};

  final Map<String, List<String>> weeklySlotLabels = {
    'Monday': [
      '5:30 AM - 6:30 AM',
      '6:00 AM - 7:00 AM',
      '6:30 AM - 7:30 AM',
  ...
  void _listenToTrainerSchedule() {
    _scheduleListener?.cancel();
    _scheduleListener = FirebaseFirestore.instance
      ...
      if (!doc.exists) {
        ...
        setState(() {
          _loadedWeeklySlots = {};
          _loadedDateOverrides = {};
        });
        return;
      }

      final data = doc.data() ?? <String, dynamic>{};
      final weekly = <String, List<String>>{};

      for (final day in weeklySlotLabels.keys) {
        final raw = data[day];
        if (raw is List) {
          weekly[day] = List<String>.from(raw);
        } else {
          weekly[day] = List<String>.from(weeklySlotLabels[day] ?? const <String>[]);
        }
      }

      final overrides = <String, List<String>>{};
      final rawOverrides = data['date_overrides'];
      if (rawOverrides is Map<String, dynamic>) {
        rawOverrides.forEach((dateKey, slots) {
          if (slots is List) {
            overrides[dateKey] = List<String>.from(slots);
          }
        });
      }
      ...
      if (!mounted) return;
      setState(() {
        _loadedWeeklySlots = weekly;
        _loadedDateOverrides = overrides;
      });
    }, onError: (e) {
      debugPrint('Failed to listen trainer schedule: $e');
    });
  }
  ...
  DateTime? _parseSlotStartForDay(DateTime day, String slotLabel) {
    try {
      final start = slotLabel.split(' - ').first.trim();
      final parsed = _parseTimeFlexible(start);
  ...
  DateTime _parseTimeFlexible(String value) {
    final normalized = value.trim();
    final upper = normalized.toUpperCase();

    if (upper.contains('AM') || upper.contains('PM')) {
      // Use explicit en_US locale so AM/PM is always recognised regardless of device locale
      try {
        ...
        try {
          ...
          return DateFormat.jm().parseStrict(normalized);
        }
      }
    }

    try {
      return DateFormat("HH:mm").parseStrict(normalized);
    } catch (_) {
      return DateFormat.jm().parseStrict(normalized);
    }
  }

  String _normalizeTimeTo24Hour(String value) {
    final parsed = _parseTimeFlexible(value.trim());
    return DateFormat("HH:mm").format(parsed);
  }

  String _normalizeSlotLabelTo24Hour(String slotLabel) {
    final parts = slotLabel.split(' - ');
    if (parts.length != 2) return slotLabel;
    return '${_normalizeTimeTo24Hour(parts[0])} - ${_normalizeTimeTo24Hour(parts[1])}';
  }

  Future<DocumentSnapshot<Map<String, dynamic>>?> _getSlotDocForLabel(
    DateTime date,
    String time,
  ) async {
    final dateKey = DateFormat('yyyy-MM-dd').format(date);
    final primaryDocId = '$dateKey|$time';
    final fallbackDocId = '$dateKey|${_normalizeSlotLabelTo24Hour(time)}';

    final primary = await FirebaseFirestore.instance
        .collection('trainer_slots')
        .doc(primaryDocId)
        .get();
    if (primary.exists) return primary;

    if (fallbackDocId != primaryDocId) {
  ...
  List<String> getSlotsForDay(DateTime date) {
    ...
    final weekly = _loadedWeeklySlots[weekday] ?? weeklySlotLabels[weekday] ?? const <String>[];
    final dateSpecific = _loadedDateOverrides[dateKey] ?? const <String>[];

    final merged = <String>[...weekly];
    for (final slot in dateSpecific) {
      if (!merged.contains(slot)) {
        merged.add(slot);
      }
    }

    List<String> result = List<String>.from(merged);

    result.sort((a, b) {
      try {
        final aStart = _parseTimeFlexible(a.split(' - ').first.trim());
        final bStart = _parseTimeFlexible(b.split(' - ').first.trim());
    ...
    if (!isSameDay(date, now)) return result;

    return result.where((slot) {
      final slotStart = _parseSlotStartForDay(date, slot);
  ...
  Future<Map<String, Map<String, dynamic>>> getSlotStatuses(DateTime date, List<String> slots) async {
    ...
    for (String time in slots) {
      // Check if the slot end time has already passed
      bool isPast = false;
      try {
        final parts = time.split(' - ');
        if (parts.length == 2) {
          final endParsed = _parseTimeFlexible(parts[1].trim());
          final endDt = DateTime(date.year, date.month, date.day, endParsed.hour, endParsed.minute);
          isPast = now.isAfter(endDt);
        }
      } catch (_) {}

      if (isPast) {
        statuses[time] = {'isFull': false, 'isBookedByUser': false, 'isPast': true};
        continue;
      }

      try {
        final doc = await _getSlotDocForLabel(date, time);
        if (doc == null || !doc.exists) {
          statuses[time] = {'isFull': false, 'isBookedByUser': false, 'isPast': false};
        } else {
          final data = doc.data() ?? <String, dynamic>{};
        ...
        // Keep slots visible even when status lookup fails for a specific item.
        statuses[time] = {'isFull': false, 'isBookedByUser': false, 'isPast': false};
        debugPrint('Slot status read failed for $time: $e');
      }
    }
    return statuses;
  }
 ...
 // Add this method to ClientBookSlot for immediate data sync
  ...
  Future<void> _cancelBooking(String docId, {bool suppressError = false}) async {
    ...
    try {
      ...
      await FirebaseFirestore.instance.runTransaction((transaction) async {
        ...
        // Preserve existing statuses for all other users
        Map<String, dynamic> statusByUser = Map<String, dynamic>.from(doc['status_by_user'] ?? {});
        ...
        statusByUser[currentUser.uid] = 'Cancelled';
  ...
  void showSlotPopup(String time) async {
    ...
    final existingSlotDoc = await _getSlotDocForLabel(selectedDate, time);
    final existingSlotData = existingSlotDoc?.data() ?? <String, dynamic>{};
    final existingBookedBy = List.from(existingSlotData['booked_by'] ?? []);
    final resolvedDocId = existingSlotDoc?.id ??
      "${DateFormat('yyyy-MM-dd').format(selectedDate)}|$time";
    final isBookedByUser = existingSlotDoc != null &&
      existingBookedBy.contains(currentUser.uid);
    final isFull = existingSlotDoc != null &&
      existingBookedBy.length >= slotCapacity;
          ...
          if (!isFull || isBookedByUser || isRescheduleFlow)
            TextButton(
              onPressed: () async {
                ...
                if (isBookedByUser) {
                  await _cancelBooking(resolvedDocId);
                  setState(() {});
                } else {
                          ...
                          FutureBuilder<Map<String, Map<String, dynamic>>>(
                            ...
                            builder: (context, snapshot) {
                              ...
                              if (snapshot.hasError) {
                                final fallbackSlots = List<String>.from(slots);
                                return Padding(
                                  ...
                                  child: GridView.builder(
                                    ...
                                    itemCount: fallbackSlots.length,
                                    itemBuilder: (context, index) {
                                      final time = fallbackSlots[index];
                              ...
                              return Padding(
                                ...
                                child: GridView.builder(
                                  ...
                                  itemBuilder: (context, index) {
                                    ...
                                    final slotInfo = statuses[time] ?? {'isFull': false, 'isBookedByUser': false, 'isPast': false};
                                    final isPast = slotInfo['isPast'] == true;
                                    final isDisabled = isPast || (slotInfo['isFull'] == true && slotInfo['isBookedByUser'] != true);
                                    final isBookedByUser = slotInfo['isBookedByUser'] == true;
