...
class _ClientPlansScreenState extends State<ClientPlansScreen> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    // Auto-refresh every 5 minutes to catch completed sessions
  ...
  Future<void> _refreshSessionCounts() async {
    try {
      ...
      for (final purchaseDoc in purchasesSnapshot.docs) {
        ...
        // Get current booked sessions count from purchase document
        final currentBookedSessions = purchaseData['bookedSessions'] as int? ?? 0;

        // Count COMPLETED sessions (past sessions OR sessions marked as completed)
        ...
        for (final slotDoc in slotsSnapshot.docs) {
          try {
            ...
            if (belongsToThisPurchase) {
              ...
              // Check if session is completed (either marked as Completed OR end time has passed)
              bool isCompleted = false;
              
              if (userSlotStatus == 'Completed') {
                isCompleted = true;
              } else if (userSlotStatus != 'Cancelled') {
                // Check if the slot end time has passed
                if (slotTime.isNotEmpty) {
                  try {
                    // Parse the end time of the slot
                    ...
                    if (now.isAfter(slotEndTime)) {
                      isCompleted = true;
                      // Also mark it as Completed in the slot document for future reference
                      if (userSlotStatus != 'Completed') {
                        await slotDoc.reference.update({
                          'status_by_user.${userId}': 'Completed'
                        });
                      }
                    }
                  } catch (e) {
                    // If parsing fails, check if the entire day has passed
                    if (slotDate.isBefore(DateTime(now.year, now.month, now.day))) {
                      isCompleted = true;
                    }
                  }
                } else if (slotDate.isBefore(DateTime(now.year, now.month, now.day))) {
                  // If no time info, check if the date has passed
                  isCompleted = true;
                }
              }
        ...
        final remainingSessions = totalSessions - completedSessionsCount;
        
        // Plan is completed ONLY when ALL sessions are completed
        final newStatus = (completedSessionsCount >= totalSessions && totalSessions > 0) ? 'completed' : 'active';
        ...
        bool needsUpdate = currentUsed != completedSessionsCount || 
        ...
        if (needsUpdate) {
          ...
          // Calculate available sessions (total - booked)
          final availableSessions = totalSessions - currentBookedSessions;
          
          Map<String, dynamic> updateData = {
            'usedSessions': completedSessionsCount,
          ...
          print('🔄 Updated purchase ${purchaseData['planName']}: '
              'Completed: $completedSessionsCount/$totalSessions, '
