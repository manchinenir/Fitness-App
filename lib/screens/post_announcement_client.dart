...
class _PostAnnouncementScreenState extends State<PostAnnouncementScreen> {
  ...
  Rect _shareOrigin(BuildContext ctx) {
    ...
    if (box != null && box.hasSize) return box.localToGlobal(Offset.zero) & box.size;
    final size = MediaQuery.of(ctx).size;
    return Rect.fromLTWH(0, size.height - 100, size.width, 100);
  }

  Future<void> _shareReferralOld(String referralCode, BuildContext ctx) async {
    ...
    await Share.share(message, subject: subject, sharePositionOrigin: _shareOrigin(ctx));
  }
  ...
  // Function to handle referral sharing with dynamic app links
  Future<void> _shareReferral(BuildContext ctx) async {
  ...
  // Function to share via specific platform with personalized link
  Future<void> _shareViaPlatform(String platform, BuildContext ctx) async {
    if (userReferralCode == null) return;
    ...
    switch (platform) {
      case 'whatsapp':
        ...
        if (await canLaunchUrl(Uri.parse(whatsappUrl))) {
          ...
          await Share.share('$message\n$referralLink', subject: "Fitness App Referral", sharePositionOrigin: origin);
        }
        break;
      case 'email':
        final String subject = "Fitness App Referral";
        ...
        if (await canLaunchUrl(Uri.parse(emailUrl))) {
          ...
          await Share.share('$message\n$referralLink', subject: subject, sharePositionOrigin: origin);
        }
        break;
    }
  }
                        ...
                        children: [
                          ...
                          ElevatedButton.icon(
                            onPressed: () {
                              ...
                              final shareText = (link != null && link.toString().isNotEmpty)
                                  ? '${data['title']}\n\n${data['message'] ?? ''}\n\n$link'
                                  : '${data['title']}\n\n${data['message'] ?? ''}';
                              Share.share(shareText, sharePositionOrigin: _shareOrigin(context));
              ...
              if (_expandedAnnouncements.contains(docId)) ...[
                ...
                Row(
                  ...
                  children: [
                    ...
                    ElevatedButton.icon(
                      onPressed: () {
                        _shareReferralOld(data['referralCode'] ?? '', context);
                      },
