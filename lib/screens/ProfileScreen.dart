...
class _ProgressTrackingPageState extends State<ProgressTrackingPage> {
  ...
  Future<void> _deleteProgressEntry(String id) async {
    final user = FirebaseAuth.instance.currentUser;
    if (user != null) {
      final previousEntries = List<Map<String, dynamic>>.from(_progressEntries);
      if (mounted) {
        setState(() {
          _progressEntries.removeWhere((entry) => entry['id'] == id);
          if (_editingEntry != null && _editingEntry!['id'] == id) {
            _cancelEdit();
          }
        });
      }
      ...
      try {
        ...
        if (!mounted) return;
        setState(() {
          _progressEntries = previousEntries;
        });
        ScaffoldMessenger.of(context).showSnackBar(
                  ...
                  children: [
                    ...
                    LayoutBuilder(
                      builder: (context, constraints) {
                        ...
                        final photoToggleButton = SizedBox(
                          width: useVerticalLayout ? double.infinity : 50,
                          child: ElevatedButton(
                            style: ElevatedButton.styleFrom(
                              backgroundColor: kPrimary,
